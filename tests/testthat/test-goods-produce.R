test_that("goods_produce_recursively() works", {
  ...
  final_demand <- get_final_demand_iotable_regional(
    iotable = get_iotable_dummy()
  ) |>
  ...
  f <- function(industry) {
    quantity <- industry |>
      ...
      dplyr::ungroup() |>
      dplyr::summarise(
        quantity_industry = sum(.data$quantity),
        .by = "input_sector"
      ) |>
      dplyr::inner_join(final_demand, by = "input_sector") |>
      dplyr::mutate(
        quantity = .data$quantity_industry + .data$quantity_final_demand,
        .keep = "unused"
      ) |>
  ...
  expect_equal(industry_new, industry, tolerance = 1e-6)
})
