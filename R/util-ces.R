...
util_ces2 <- function(
  ...
  util_ces(
    substitution = (elasticity_of_substitution - 1) /
      elasticity_of_substitution,
    homogeneity = homogeneity,
    efficiency = efficiency,
    weights = weights,
...
util_gradient.util_ces <- function(f, quantities, ...) {
  ...
  gradient_utility <- f$efficiency *
    f$homogeneity *
    sum(f$weights * quantities^f$substitution, na.rm = TRUE)^(f$homogeneity /
      f$substitution -
      1) *
    f$weights *
    quantities^(f$substitution - 1)
...
util_calibrate.util_ces <- function(f, prices, quantities, ...) {
  ...
  f$efficiency <- sum(prices * quantities) /
    sum(weights * quantities^f$substitution)^(1 / f$substitution)
  f
}

#' @export
util_demand_marshallian.util_ces <- function(
  f,
  prices,
  income,
  gradient = FALSE,
  ...
  if (gradient) {
    diag(
      f$weights^(1 / (1 - f$substitution)) /
        (f$substitution - 1) *
        prices^(1 / (f$substitution - 1) - 1) *
        income /
        sum(
          f$weights^(1 / (1 - f$substitution)) *
            prices^(f$substitution / (f$substitution - 1))
        )
    ) +
      outer(
        f$weights^(1 / (1 - f$substitution)) *
          prices^(1 / (f$substitution - 1)),
        -income /
          sum(
            f$weights^(1 / (1 - f$substitution)) *
              prices^(f$substitution / (f$substitution - 1))
...
obj_sum.util_ces <- function(x) {
  if (inherits(x, "util_homothetic")) {
    ...
    paste0(
      type_sum(x),
      "(",
      big_mark(x$substitution),
      ", ",
      big_mark(x$homogeneity),
      ")"
    )
  }
}
