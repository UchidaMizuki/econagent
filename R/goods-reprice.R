...
#'
#' @export
goods_reprice <- function(data, prices = NULL) {
  ...
  for (i in seq_along(prices)) {
    data <- data |>
      dplyr::rows_update(prices[[i]], by = setdiff(names(prices[[i]]), "price"))
  }

  data |>
    timbr::traverse(\(x, y) {
      quantities <- util_demand(x$utility[[1]], y$price, utility = x$quantity)
